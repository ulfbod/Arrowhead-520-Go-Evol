...
// TestReissue_UnrevokesAndRestoresVisibility verifies that Reissue clears the
// revoked flag and makes the record visible in GetAll again.
func TestReissue_UnrevokesAndRestoresVisibility(t *testing.T) {
	ca := newTestCA(t)
	...
	for _, r := range records {
		if r.CN == "sp-reissue" {
			found = true
			if r.Revoked {
				t.Error("reissued cert should not be revoked")
			}
		}
	}
	if !found {
