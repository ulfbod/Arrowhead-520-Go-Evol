...
- **Credential verification is a stub**: `ENABLE_IDENTITY_CHECK` prevents name spoofing but not token theft — any system can log in as any `systemName` since credential verification is not implemented. The E2E identity tests cover the orchestration mechanics, not the security strength of the credential check.
